define(["exports", "./../emby-apiclient/connectionmanager.js", "./../dom.js", "./../common/globalize.js", "./../layoutmanager.js", "./../common/itemmanager/itemmanager.js", "./../common/imagehelper.js", "./../dialoghelper/dialoghelper.js", "./../emby-elements/emby-button/emby-button.js", "./../emby-elements/emby-scroller/emby-scroller.js", "./../emby-elements/emby-itemscontainer/emby-itemscontainer.js", "./../approuter.js", "./../focusmanager.js", "./../listview/listview.js", "./../shortcuts.js", "./../common/textencoding.js"], function(_exports, _connectionmanager, _dom, _globalize, _layoutmanager, _itemmanager, _imagehelper, _dialoghelper, _embyButton, _embyScroller, _embyItemscontainer, _approuter, _focusmanager, _listview, _shortcuts, _textencoding) {
    ...
    function autoFocusInternal(dlg, options) {
        ...
        focused || null != options && options.skipIfNotEnabled && !_focusmanager.default.isAutoFocusEnabled() || _focusmanager.default.autoFocus(dlg, options)
    }
    ...
    function refreshItemsContainer() {
        ...
        return itemsContainer.resume ? onItemsContainerUpgraded.call(this) : function(instance, itemsContainer) {
    ...
    function getListViewItemFromInputItem(option, options) {
        var isSelected, item = {
            ...
            selectedValuesIncludesId = (options.nameProperty && null != option[options.nameProperty] && (item.Name = option[options.nameProperty]), options.getItemId ? item.Id = options.getItemId(option) : null != option.Id ? item.Id = option.Id : null != option.id && "" !== option.id ? item.Id = option.id : null != option.value ? item.Id = option.value : item.Id = option.Name || option.name, null != item.Id && options.selectedValuesClone.includes(item.Id));
        return options.multiple ? (isSelected = options.emptyValueSetEqualsAll && !options.selectedValuesClone.length || selectedValuesIncludesId, item.Selected = isSelected) : item.Selected = option.selected || option.Selected || selectedValuesIncludesId, item.Icon = option.icon || option.Icon || (options.useIconForSelection && item.Selected && options.hasItemSelectionState ? "&#xe5ca;" : null), item.iconClass = option.iconClass, item.asideText = option.asideText, item.asideIcon = option.asideIcon || (options.useAsideIconForSelection && item.Selected && options.hasItemSelectionState ? "&#xe5ca;" : null), item.executeActionOnClick = option.executeActionOnClick, item.ShortOverview = options.getItemSecondaryText ? options.getItemSecondaryText(option) : option.secondaryText || option.title || option.ShortOverview, item.originalItem = option, item
    }

    function getItemsFn(options) {
        return function(query) {
            if (query = query || {}, options.getItems) return function(options, query) {
                return options.getItems(query).then(function(result) {
                    for (var items = result.Items.slice(0), i = 0, length = items.length; i < length; i++) items[i] = getListViewItemFromInputItem(items[i], options);
                    return result.Items = items, result
                })
            }(options, query);
            for (var inputItems = options.items || [], items = [], i = 0, length = inputItems.length; i < length; i++) {
            ...
            return null != query.Limit && items.length > query.Limit && (items.length = Math.min(items.length, query.Limit)), Promise.resolve({
                Items: items,
    ...
    function onItemsChecked(options) {
        for (var items = options.items, checked = options.checked, i = 0, length = items.length; i < length; i++) items[i].Selected = checked;
    ...
    function getListOptions() {
        return {
            ...
            options: function(options) {
                var listItemBodyClass, iconClass, asideTextClass, menuItemClass = "actionSheetMenuItem",
                    fields = (options.menuItemClass && (menuItemClass += " " + options.menuItemClass), options.iconRight && (menuItemClass += " actionSheetMenuItem-iconright"), options.multiple ? listItemBodyClass = "actionsheetListItemBody-multiple" : (options.hasItemIcon || options.hasItemSelectionState || (menuItemClass += " actionSheetMenuItem-noicon"), listItemBodyClass = "actionsheetListItemBody", options.iconRight && (listItemBodyClass += " actionsheetListItemBody-iconright"), iconClass = "actionsheetMenuItemIcon listItemIcon listItemIcon-transparent md-icon", asideTextClass = "actionSheetItemAsideText"), (fields = options.fields) || ["Name"]),
                    noTextWrap = (options.multiple && fields.unshift("ItemCheckbox"), options.noTextWrap || options.hasItemAsideText),
                    fields = {
                        action: options.multiple ? "togglecheckbox" : "custom",
                        fields: fields,
                        draggable: options.enableReordering || !1,
                        dragReorder: options.enableReordering || !1,
                        ...
                        border: _layoutmanager.default.tv ? options.border : !1 !== options.border,
                        artist: options.artist,
                        asideTextClass: asideTextClass,
                        asideIconClass: ((asideTextClass || "") + " actionSheetItemAsideIcon").trim(),
                        image: !options.multiple && (!0 === options.hasItemIcon || !0 === options.hasItemImage || !0 === options.hasItemSelectionState),
    ...
    Object.defineProperty(_exports, "__esModule", {
        value: !0
    }), _exports.default = void 0, require(["css!modules/actionsheet/actionsheet.css", "material-icons"]), ActionSheet.prototype.show = function(options) {
        const item = options.item;
        const isVideo = item && (item.Type === 'Movie' || item.Type === 'Episode' || item.Type === 'Video' || item.Type === 'Series' || item.Type === 'Season');
        if (isVideo && options.items) {
            options.items.push({
                Name: 'Report Issue',
                Id: 'reportIssue',
                icon: 'sync_problem'
        ...
        for (var inputItems = options.items || [], i = 0, length = inputItems.length; i < length; i++)
            if (inputItems[i].dividerAfter) {
                options.hasDivider = !0;
                break
            } _layoutmanager.default.tv || (options.clearBorder = !options.hasDivider);
        var enablePaddingTop, enablePaddingBottom, dialogOptions = {
                removeOnClose: !0,
                ...
                setDialogSize: options.setDialogSize || !1,
            ...
            useVirtualScroller = null != options.getItems && options.enableVirtualScroller,
            isFullscreen = ((options.useVirtualScroller = useVirtualScroller) && (dialogOptions.fixedSize = !0), "fullscreen" === dialogOptions.size),
            dlg = (options.hasItemImage && (options.useListViewSizing = !0), isFullscreen || (enablePaddingInline = enablePaddingBottom = enablePaddingTop = !0), _dialoghelper.default.createDialog(dialogOptions)),
            instance = this,
            dialogOptions = ((instance.dlg = dlg).classList.add("actionSheet"), _layoutmanager.default.tv),
            forceAutoFocus = (dialogOptions && dlg.classList.add("actionSheet-focusscale"), _layoutmanager.default.tv || options.hasItemSelectionState),
            forceAutoFocus = ((forceAutoFocus || isFullscreen) && (dlg.classList.add("dlg-autofocus"), forceAutoFocus) && dlg.classList.add("dlg-autofocus-force"), isFullscreen || (useVirtualScroller && dlg.classList.add("actionSheet-virtualscroll"), _layoutmanager.default.tv || (dlg.classList.add("actionSheet-automobile", "actionSheet-largefont"), options.multiple && dlg.classList.add("actionSheet-smallfont")), !1 !== options.blur && _dom.default.allowBackdropFilter() && dlg.classList.add("dialog-blur")), options.dialogClass && dlg.classList.add(options.dialogClass), "");
        options.hasItemSelectionState ? options.hasItemIcon ? (options.iconRight = !1, options.useAsideIconForSelection = !0) : (options.iconRight = !1, options.useIconForSelection = !0) : !options.hasItemIcon || options.hasItemAsideIcon || options.hasItemAsideText || !1 === options.iconRight || (options.iconRight = !0), options.iconRight && !isFullscreen && (options.useCustomImageContainerWidth = !1), isFullscreen ? forceAutoFocus += '<button is="paper-icon-button-light" class="btnCloseActionSheet hide-mouse-idle-tv" tabindex="-1"><i class="md-icon autortl">&#xe5cd;</i></button>' : useVirtualScroller && (forceAutoFocus += '<button is="paper-icon-button-light" class="btnCloseActionSheet dialogbutton-hidenotfullscreen hide-mouse-idle-tv" tabindex="-1"><i class="md-icon autortl">&#xe5cd;</i></button>');
        (!options.title || options.hasItemIcon || options.hasItemImage || options.hasItemSelectionState) && !isFullscreen || dlg.classList.add("actionsheet-centered");
        for (var selectNoneText, enablePaddingInline, previewHtml = "", previewHtml = (options.item && (previewHtml = function(options, item) {
                var html = "",
                    apiClient = (html += '<div class="actionsheetItemPreview flex flex-direction-row">', _connectionmanager.default.getApiClient(item)),
                    imageUrl = (imageUrlInfo = _imagehelper.default.getImageUrl(item, apiClient, {
                        height: 60,
                        width: 200
                    })).imgUrl,
                    imgClass = "",
                    round = "MusicArtist" === item.Type,
                    html = imageUrl ? (imageUrlInfo = imageUrlInfo.aspect || 1, imgClass = (imgClass += " actionsheetItemPreviewImage-bg") + " actionsheetItemPreviewImage-bg-" + (shape = _imagehelper.default.getShapeFromAspect(imageUrlInfo)), round && "square" === shape && (imgClass += " actionsheetItemPreviewImage-round"), html + ('<div class="' + imgClass.trim()) + '" style="aspect-ratio:' + imageUrlInfo + ";background-image:url(" + imageUrl + ');"></div>') : (imgClass += " actionsheetItemPreviewImage-iconcontainer", round && (imgClass += " actionsheetItemPreviewImage-round"), (html = html + ('<div class="' + imgClass.trim()) + '"><i class="actionsheetItemPreviewImage-icon md-icon autortl">') + _itemmanager.default.getDefaultIcon(item) + "</i></div>"),
                    lines = (html += '<div class="actionsheetItemPreviewContent">', []),
                    shape = (item.SeriesName ? item.SeriesId ? lines.push(getTextLinkButton({
                        Type: "Series",
                        Id: item.SeriesId,
                        Name: item.SeriesName,
                        IsFolder: !0,
                        ServerId: item.ServerId
                    })) : lines.push(_textencoding.default.htmlEncode(item.SeriesName || "")) : "Program" === item.Type && lines.push(_textencoding.default.htmlEncode(item.Name || "")), _textencoding.default.htmlEncode(_itemmanager.default.getDisplayName(item, {}) || "")),
                    imageUrlInfo = '<div class="actionsheetPreviewTextItem mediaInfoItem">' + shape + "</div>",
                    round = ("Audio" !== item.MediaType || _layoutmanager.default.tv || !1 === options.linkToItem || (imageUrlInfo += '<a is="emby-linkbutton" title="' + shape + '" style="font-weight:inherit;" href="' + _approuter.default.getRouteUrl(item) + '" class="flex-shrink-zero actionsheetPreviewTextItem mediaInfoItem button-link button-link-color-inherit"><i class="md-icon" style="font-size:160%;">&#xe88e;</i></a>'), lines.push(imageUrlInfo), "Server" === item.Type && apiClient && apiClient.serverVersion() && lines.push(apiClient.serverVersion()), "User" === item.Type && options.showServerName && (imageUrl = apiClient.serverName()) && lines.push(_textencoding.default.htmlEncode(imageUrl)), item.IsFolder && item.AlbumArtists && item.AlbumArtists.length ? lines.push(getTextLinkButton({
                        Type: "MusicArtist",
                        Id: item.AlbumArtists[0].Id,
                        Name: item.AlbumArtists[0].Name,
                        ServerId: item.ServerId
                    })) : item.ArtistItems && item.ArtistItems.length ? lines.push(getTextLinkButton({
                        Type: "MusicArtist",
                        Id: item.ArtistItems[0].Id,
                        Name: item.ArtistItems[0].Name,
                        ServerId: item.ServerId
                    })) : item.AlbumArtists && item.AlbumArtists.length && lines.push(getTextLinkButton({
                        Type: "MusicArtist",
                        Id: item.AlbumArtists[0].Id,
                        Name: item.AlbumArtists[0].Name,
                        ServerId: item.ServerId
                    })), []);
                item.Album && item.AlbumId ? round.push(getTextLinkButton({
                    Type: "Photo" === item.MediaType ? "PhotoAlbum" : "MusicAlbum",
                    Id: item.AlbumId,
                    Name: item.Album,
                    ServerId: item.ServerId
        ...
        return new Promise(function(resolve, reject) {
            ...
            function onValueChange(refreshItems) {
                hasSelectionChanged = !0, options.onChange && options.onChange(getResolveResult()), options.refreshItemsOnChange && !1 !== refreshItems && (itemsContainer.scrollToIndex(0, {}, itemsContainer.contains(document.activeElement)), itemsContainer.refreshItems())
            }
            itemsContainer.addEventListener("action-null", function(e) {
                const selectedMenuItem = e.detail.item;
                if (selectedMenuItem && selectedMenuItem.Id === 'reportIssue') {
                    ...
                    if (description !== null && description.trim() !== "") {
                        ...
                        apiClient.ajax({
                            ...
                            alert('Failed to report issue. See browser console for details.');
                        });
                    }
                    reject();
                    isResolved = true;
                ...
                var resolveResult;
                options.multiple || (e = e.detail.item, e = (null == (selectedItem = e) ? void 0 : selectedItem.originalItem) || selectedItem, resolveResult = options.resolveWithSelectedItem ? e : null == selectedItem ? void 0 : selectedItem.Id, !e.executeActionOnClick && (!options.resolveOnClick || options.resolveOnClick.indexOf && -1 === options.resolveOnClick.indexOf(null == selectedItem ? void 0 : selectedItem.Id)) || (resolve(resolveResult), isResolved = !0), _dialoghelper.default.close(dlg))
            }), itemsContainer.fetchData = getItemsFn(options), itemsContainer.getListOptions = getListOptions.bind(options), null != (_dlg$querySelector = dlg.querySelector(".btnActionSheetSelectNone")) && _dlg$querySelector.addEventListener("click", function() {
                options.selectedValuesClone = [], this.classList.contains("dlg-close") ? (onValueChange(!1), _dialoghelper.default.close(dlg)) : onValueChange()
            }), itemsContainer.addEventListener("change", function(e) {
                hasSelectionChanged = !0;
                var arr, value, e = e.target.closest("input"),
                    itemValue = _shortcuts.default.getItemFromChildNode(e, null, this).Id;
                arr = options.selectedValuesClone, value = itemValue, -1 < (value = arr.indexOf(value)) && arr.splice(value, 1), e.checked && options.selectedValuesClone.push(itemValue), onValueChange()
            }), itemsContainer.addEventListener("itemdrop", function(e) {
                e.preventDefault();
                var e = e.detail,
                    item = e.items[0],
                    item = options.selectedValuesClone.indexOf(item.Id); - 1 === item || e.newIndex >= options.selectedValuesClone.length || (options.selectedValuesClone = function(arr, old_index, new_index) {
                    if (new_index >= arr.length) throw new Error("out of bounds");
                    return arr.splice(new_index, 0, arr.splice(old_index, 1)[0]), arr
                }(options.selectedValuesClone, item, e.newIndex), onValueChange())
            }), itemsContainer.afterRefresh = function() {
                itemsContainer.virtualScroller || callUpdateElement(itemsContainer, options)
            };
            var openingPromise = new Promise(function(resolve, reject) {
                    dlg.addEventListener("open", resolve)
                }),
                openedPromise = new Promise(function(resolve, reject) {
            ...
            (useVirtualScroller ? Promise.resolve() : refreshItemsContainer.call(dlg)).then(function() {
                ...
                return dlg.dialogOptions.setDialogSize = !0, _dialoghelper.default.positionDialog(dlg), options.timeout && (timeout = setTimeout(function() {
                    _dialoghelper.default.close(dlg)
                }, options.timeout)), useVirtualScroller && (refreshPromise = refreshItemsContainer.call(dlg)), openingPromise.then(function() {
                    _dialoghelper.default.positionDialog(dlg)
                }), openedPromise.then(function() {
                    return (refreshPromise || Promise.resolve()).then(function() {
                        var skipIfNotEnabled;
                        this.classList.contains("dlg-autofocus") && (skipIfNotEnabled = !this.classList.contains("dlg-autofocus-force"), autoFocusInternal(this, {
                            skipIfNotEnabled: skipIfNotEnabled
                        }))
                    }.bind(dlg)), itemsContainer.virtualScroller && itemsContainer.virtualScroller.onResized(), closePromise.then(function() {
                        var resolveResult;
                        if (instance.dlg = null, timeout && (clearTimeout(timeout), timeout = null), !isResolved) return null != (resolveResult = getResolveResult()) ? (options.callback && options.callback(resolveResult), resolveResult) : Promise.reject()
                    })
                })
            }).then(resolve, reject)
        })
    }, ActionSheet.prototype.refreshItems = function() {
        var _this$dlg = null == (_this$dlg = this.dlg) ? void 0 : _this$dlg.querySelector(".itemsContainer");
        ...
        var _this$dlg2 = null == (_this$dlg2 = this.dlg) ? void 0 : _this$dlg2.querySelector(".itemsContainer");
        _this$dlg2 && _this$dlg2.pause(), this.dlg = null
    };
    _exports.default = {
